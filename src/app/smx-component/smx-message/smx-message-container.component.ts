...
import { toCssPixel } from './core/util/convert';

import { SmxMessageConfig, NZ_MESSAGE_CONFIG, NZ_MESSAGE_DEFAULT_CONFIG } from './smx-message-config';
...
@Component({
  ...
  templateUrl: './smx-message-container.component.html'
})
export class SmxMessageContainerComponent {
  messages: NzMessageDataFilled[] = [];
  config: Required<SmxMessageConfig>;
  top: string | null;

  constructor(
    protected cdr: ChangeDetectorRef,
    @Optional() @Inject(NZ_MESSAGE_DEFAULT_CONFIG) defaultConfig: SmxMessageConfig,
    @Optional() @Inject(NZ_MESSAGE_CONFIG) config: SmxMessageConfig
  ) {
    this.setConfig({ ...defaultConfig, ...config });
  }

  setConfig(config: SmxMessageConfig): void {
